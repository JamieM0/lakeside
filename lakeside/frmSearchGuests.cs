    ...
    public partial class frmSearchGuests : Form
    {
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (searchType == "guest")
            {
                ...
                //dgResults.Columns.Remove(dgResults.Columns[9]);
                //dgResults.Columns.Remove(dgResults.Columns[10]);
            }
        ...
        {
            CenterToScreen();
            dgResults.Font = new Font("Segoe UI", 12);
            btnEditElement.Text = "";
            btnRemoveElement.Text = "";
            btnEditElement.BackgroundImageLayout = ImageLayout.Zoom;
            btnRemoveElement.BackgroundImageLayout = ImageLayout.Zoom;
            if (searchType=="guest")
            {
            ...
            {
                btnEditElement.BackgroundImage = Properties.Resources.EditPodButton;
                btnRemoveElement.BackgroundImage = Properties.Resources.RemovePodButton;
                btnRemoveElement.Visible = false;
            }
            else if (searchType == "course")
            {
                btnEditElement.BackgroundImage = Properties.Resources.EditCourseButton;
        ...
        private void btnEditElement_Click(object sender, EventArgs e)
        {
            if (searchType == "guest")
            {
                try
                {
                    if (!fromPodBooking)
                    {
                        LakesideDAL dal = new LakesideDAL();
                        System.Windows.Forms.Button btn = (System.Windows.Forms.Button)sender;
                        int guestID = int.Parse(dgResults.SelectedRows[0].Cells[8].Value.ToString());
                        Guest g = dal.GuestLookup(guestID);
                        Hide();
                        new frmAddGuest(g, txtSearch.Text).Show();
                    }
            ...
            else if (searchType == "pod")
            {
                try
                {
                    ...
                    System.Windows.Forms.Button btn = (System.Windows.Forms.Button)sender;
                    string btnName = btn.Name;
                    int podID = int.Parse(dgResults.SelectedRows[0].Cells[6].Value.ToString());
                    Pod p = dal.PodLookup(podID);
                    Hide();
                    new frmAddPod(p, txtSearch.Text).Show();
                }
            ...
            else if (searchType == "course")
            {
                try
                {
                    CourseDAL dal = new CourseDAL();
                    System.Windows.Forms.Button btn = (System.Windows.Forms.Button)sender;
                    int courseID = int.Parse(dgResults.SelectedRows[0].Cells[0].Value.ToString());
                    Course c = dal.CourseLookup(courseID);
                    Hide();
                    new frmAddCourse(c, txtSearch.Text).Show();
                }
